...
const App: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const verifiedHistoricalData = historicalData.filter(d => d.date.startsWith(currentYear));

      if(verifiedHistoricalData.length !== historicalData.length) {
        console.warn("Filtered out stale data from AI response.");
      }

      const cleanedHistoricalData = verifiedHistoricalData
